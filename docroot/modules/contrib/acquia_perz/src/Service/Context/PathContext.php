...
use Drupal\acquia_perz\Service\Helper\PathMatcher;
...
class PathContext extends BaseContext implements CacheableDependencyInterface {
   ...
   * Path matcher.
   *
   * @var \Drupal\acquia_perz\Service\Helper\PathMatcher
   */
  private $pathMatcher;
   ...
   *   The request stack.
   * @param \Drupal\acquia_perz\Service\Helper\PathMatcher $pathMatcher
   *   The path matcher.
   * @param \Drupal\acquia_connector\Subscription $subscription
   *   Acquia Subscription.
   */
  public function __construct(ConfigFactoryInterface $config_factory, CurrentPathStack $current_path_stack, RequestStack $request_stack, PathMatcher $pathMatcher, Subscription $subscription) {
    $this->settings = $config_factory->get('acquia_perz.settings');
  ...
  public function shouldAttach() {
    ...
    $asset_url = $this->settings->get('api.assets_url');

    // If subscription wasn't set, don't attach.
    if ($this->subscription === []) {
      return FALSE;
    }

    if (!SettingsHelper::isValidCredential($this->subscription, $site_id, $asset_url)) {
      return FALSE;
    }
    ...
    // Should not attach if override_lift_meta_tags config is not set.
    if (!PerzHelper::shouldOverrideLiftSettings()) {
      return FALSE;
    }
